...
%\footnote{このことより次がわかる. 「$\mathscr{B}$が問題の条件を満たせば, 任意の位相$\mathscr{U}$で$\mathscr{B}$を開集合系の基(開基)とするものは,$\mathscr{U}_{\mathscr{B}} $に限られる.」}
\end{enumerate}
...
%$d_n>0$かつ$\{ d_n\}_{n=1}^{\infty}$と$\{x_n + d_n \}_{n=1}^{\infty}$は単調減少である. つまり$d_1 >  d_2 > \cdots >0$と$x_1 + d_1 > x_2 + d_2 > \cdots$を満たす. 
...
%\item$^{*}$  $X$を位相空間とし, $A \subset X$を部分空間とする. 
...
次の問いに答えよ. 
  ...
  \setlength{\itemsep}{0pt} 
	...
	\item $ A=\{ (x,y)\in \R^2 | x+y=1\}$とし, $A$に$(\R^2,\mathscr{U})$の相対位相$\mathscr{U}_A$を入れる. このとき$\mathscr{U}_A$は離散位相であることを示せ. また$A$は可分でないことを示せ. つまり可分空間の部分空間は必ずしも可分ではない. 
	\item $(\R^2,\mathscr{U})$は第2可算公理を満たさないことを示せ. また距離化可能ではないことを示せ.
	\item $(\R^2,\mathscr{U})$は$T_3$空間であることを示せ. 
	\end{enumerate}
	
\item  $^{*}$ 引き続き\ref{Sor2}の位相空間$(\R^2,\mathscr{U})$を考える. $\Q$を有理数の集合とし, 
$$L := \{ (x, -x )\in \R^2 | x \in \R\}, \quad K := \{ (x, -x )\in \R^2 | x \in \Q\}$$
とおく. この問題では$(\R^2,\mathscr{U})$は$T_4$空間ではないことを示す. 次の問いに答えよ. 
\begin{enumerate}
	\setlength{\parskip}{0cm} 
  \setlength{\itemsep}{0pt} 
\item $K$と$L \setminus K$はともに$(\R^2,\mathscr{U})$において閉集合であることを示せ. 
...
\begin{enumerate}[label=(\roman*)]
\item $x_1 =0$かつ$\{x_n \}_{n=1}^{\infty}$は単調増加である. つまり$x_1 < x_2 < \cdots$を満たす.
\item $d_n>0$かつ$\{ d_n\}_{n=1}^{\infty}$と$\{x_n + d_n \}_{n=1}^{\infty}$は単調減少である. つまり$d_1 >  d_2 > \cdots >0$と$x_1 + d_1 > x_2 + d_2 > \cdots$を満たす. 
\item 任意の自然数$i$について, $[x_{2i+1}, x_{2i+1}+ d_{2i+1}) \times [-x_{2i+1}, -x_{2i+1}+ d_{2i+1})\subset U_1$.
\item 任意の自然数$i$について, $[x_{2i}, x_{2i}+ d_{2i}) \times [-x_{2i}, -x_{2i}+ d_{2i})\subset U_2$.
\end{enumerate}
\item (b)を用いて$U_1 \cap U_2  \neq \varnothing $であることを示せ. 以上より$K$と$L \setminus K$は互いに交わらない開集合で分離することはできず, $T_4$空間ではない.
\end{enumerate}
\ref{Sor1}により$T_4$空間の直積は必ずしも$T_4$空間ではなく, \ref{Sor2}より$(\R^2,\mathscr{U})$は$T_3$空間であるが$T_4$空間ではない例である. 
\end{enumerate}
...
\item $^{*}$ 次の問いに答えよ.
	 \begin{enumerate}
	 	\setlength{\parskip}{0cm} 
  \setlength{\itemsep}{0pt} 
 	\item \ref{Zariski_topology}の空間は$T_0$空間であるが, $T_1$空間ではないことを示せ.
	\item \ref{cofinite}の空間は$T_1$空間であるが, $T_2$空間ではないことを示せ.
	\item \ref{Furstenberg}の空間は$T_2$空間であるが, $T_3$空間ではないことを示せ.
\end{enumerate}
...
\xymatrix@C=25pt@R=20pt{
%\text{距離空間}\ar@{=>}[d] && &\\
%\text{距離空間}\ar@{=>}[r] &\text{$T_4$(正規ハウスドルフ)} \ar@{=>}[d] \ar@{=>}[r] &\text{$T_3$ (正則ハウスドルフ)}\ar@{=>}[d] \ar@{=>}[r]&\text{$T_2$ (ハウスドルフ)}\ar@{=>}[r] &\text{$T_1$} \ar@{=>}[d] \\
...
%%選択公理を示すには, 「ある写像$$f : \Lambda \to \bigcup_{\lambda \in \Lambda} X_{\lambda}$$で任意の$\lambda \in \Lambda$について$f(\lambda) \in X_{\lambda}$となる」ものが存在することを示せば良い. 
...
%\footnote{(iii)から(i)がかなり難しい. 難しければ(i) $\Rightarrow$ (ii) $\Rightarrow$ (iii)だけを発表しても良い. }
\begin{enumerate}[label=(\roman*)]
 \setlength{\parskip}{0cm}
  \setlength{\itemsep}{0pt} 
\item  $X$はコンパクトである.
...
%ただ中には意外と解けるものもある.実際\ref{stone}〜\ref{foliation}は去年の演習で解かれた. 
...
%\footnote{現時点では0.5点以上であるとしかいえない. つまり$^{*}$がついている問題だが0.5点しかもらえない場合もある. なお難しい位相空間もあるので$*$をつけた.} 
%今回答えやすいように難易度別に分類した. 難易度が高い方が配点も高いものとする.
...
\end{itemize}
 ...
 %\ref{Furstenberg},\ref{Sor2}, \ref{longray}
%で出てきた位相空間のハウスドルフ性・コンパクト性・連結性を各々調べよ.なおこの問題は何回も答えて良いし複数人が分担して解答してもよい. また選んだ空間によって配点が異なる. \footnote{現時点では0.5点以上であるとしかいえない. つまり$^{*}$がついている問題だが0.5点しかもらえない場合もある. なお難しい位相空間もあるので$*$をつけた.}
