...
\item $* $\label{HCC}次の空間のハウスドルフ性・連結性・コンパクト性を各々調べよ. なおこの問題は何回も答えて良いし複数人が分担して解答してもよい. また選んだ空間によって配点が異なる. \footnote{現時点では0.5点以上であるとしかいえない. つまり$^{*}$がついている問題だが0.5点しかもらえない場合もある. なお難しい位相空間もあるので$*$をつけた.} 
今回答えやすいように難易度別に分類した. 難易度が高い方が配点も高いものとする.\footnote{ただし\ref{realproj}のように演習問題でハウスドルフなどを示している場合はその事実を使って良いものとする. もしその問題が解かれていない場合はその問題を先に解くこと. この難易度は全ての問題が解かれていると仮定した場合の難易度である.} 
\begin{itemize}
\setlength{\parskip}{0cm} 
  \setlength{\itemsep}{0cm} 
\item[難易度 Easy$\sim$Normal] \ref{conti}, \ref{point}, \ref{usc}, \ref{scat}, \ref{Sorgenfrey},
 $\R\mathbb{P}^n$と$\C\mathbb{P}^n$(\ref{realproj}と\ref{cpxproj}), グラスマン多様体(\ref{grassmann}), $GL(n,\R)$と$GL(n,\R)_{>0}$(\ref{gl}), $SO(n,\R)$ (\ref{son}), 
\item[難易度 Hard]  \ref{Zariski_topology},  \ref{Furstenberg}, \ref{Sor2}, \ref{longray}, $(\Q_p, d_p)$(\ref{Qp}), $\Z_p$ (\ref{Zp})
\end{itemize}
